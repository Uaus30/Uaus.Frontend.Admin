...
function KpiCard({ label, value, hint, highlight, negative }: KpiCardProps) {
  const valueColor = highlight ? (negative ? "text-destructive" : "text-emerald-600") : "text-foreground";

  return (
    <div className={`rounded-lg border p-3 ${highlight ? "border-primary/40 bg-primary/5" : "bg-muted/20"}`}>
      ...
      {warnings && warnings.length > 0 && (
        <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 space-y-1">
          {warnings.map((warning) => (
            <p key={warning} className="flex items-start gap-2 text-sm text-amber-600 dark:text-amber-400">
              ...
              <TableBody>
                {closing.shares.map((share) => (
                  <TableRow key={share.partnerId}>
                    ...
                    <TableCell className="text-right">{formatPercentage(share.percentage)}</TableCell>
                    <TableCell
                      className={`text-right font-semibold ${share.amount < 0 ? "text-destructive" : ""}`}
                    >
