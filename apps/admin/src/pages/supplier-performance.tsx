        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-2.5">
          ...
          {relatorio && (
            <span className="text-[13px] text-muted-foreground">
              {relatorio.totals.activeSuppliers} de {relatorio.totals.totalSuppliers} fornecedores venderam no
              período
            </span>
          )}
        </div>
      ...
      {relatorio && (
        <>
          ...
          <p className="mt-3 border-t border-dashed border-border pt-4 text-xs leading-relaxed text-muted-foreground">
            ...
            {formatPercent(relatorio.parameters.turnoverWeight * 100, 0)}, quanto do estoque saiu no período,
            alvo {formatPercent(relatorio.parameters.turnoverTarget, 0)}) ·{" "}
