    ...
    <div className="flex flex-col gap-4">
      ...
      {relatorio && relatorio.totals.products === 0 && !tela.isLoading && (
        <Card className="border-border/60 p-10 text-center text-sm text-muted-foreground">
          Nenhum produto vendeu nem tem saldo em estoque no período escolhido — não há desempenho a comparar.
        </Card>
      )}
      ...
      {relatorio && relatorio.totals.products > 0 && (
        <>
          ...
          <p className="mt-2 border-t border-dashed border-border pt-4 text-xs leading-relaxed text-muted-foreground">
            <strong className="text-foreground/80">Como a nota é calculada</strong> — média ponderada de seis
            componentes, cada um de 0 a 100: <strong className="text-foreground/80">giro</strong> (
            ...
            {Math.round(relatorio.parameters.liquidityWeight * 100)}%, em quanto tempo o saldo sai, contra os{" "}
            {formatInteger(Math.round(relatorio.parameters.storeCoverageDays))} dias da loja). Quem não vendeu
            no período zera as quatro primeiras, e são as duas últimas que o posicionam —{" "}
            <strong className="text-foreground/80">é a nota que ordena as duas tabelas</strong>, e ela precisa
            desempatar até entre os parados. <strong className="text-foreground/80">Capital em risco</strong>{" "}
            continua sendo o custo na prateleira ponderado pela parte de venda da nota, mais o prejuízo já
            realizado: ele responde "quanto", depois que a ordem já respondeu "quem".
          </p>
        </>
      )}
    </div>
